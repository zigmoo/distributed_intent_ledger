#!/bin/bash
# file path: _shared/scripts/url_tool.sh (canonical — DIL active)
# mirror: /org/platform/scripts/bin/url_tool (work-domain symlink)
#
...
#
# Usage:
#   url_tool ticket WORK-12050
#   url_tool comment WORK-12050 1492390
#   url_tool smax 83736206
#   url_tool mr 42 --repo it/data-management/Platform/projects/scripts
#   url_tool pipeline 98765 --repo it/data-management/Platform/projects/scripts
#   url_tool systems
#
# Output modes:
...
#
# Contract:
#   This tool and DeployTool's Tickets screen are co-consumers of the
#   ticket_systems[].url_templates schema in domain_registry.json.
#   See WORK-12050 and WORK-12011 for design details.
#
# Exit codes:
#   0  Success
#   1  General error
...
#   2. /org/platform/data/url_tool/domain_registry.json (scripts library — works on remote servers)
#   3. $HOME/Documents/dil_agentic_memory_0001/_shared/_meta/domain_registry.json (DIL — local dev machines)
_resolve_registry() {
    if [[ -n "${URL_TOOL_REGISTRY:-}" ]]; then echo "$URL_TOOL_REGISTRY"; return; fi
    local scripts_lib="/org/platform/data/url_tool/domain_registry.json"
