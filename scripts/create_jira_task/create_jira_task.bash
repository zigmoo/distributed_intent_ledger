...
#   --epic EPIC-KEY       Epic Link (e.g. DMDI-8850)
#
# DIL options (passed to task_tool create):
...
JIRA_TOOL="/az/talend/scripts/bin/jira_tool"
...
if (( DRY_RUN )); then
  ...
  echo "[dry-run] Would run: task_tool create --domain work --title \"$SUMMARY\" --project \"$PROJECT\" --task-id <JIRA-KEY>"
  exit 0
fi
...
dil_output=$(task_tool --base "$BASE" create "${dil_args[@]}" 2>&1)
dil_exit=$?
...
# Parse DIL path from output: "OK | DMDI-11906 | work | todo | <path>"
