...
Subcommands:
  sync
    ...
    Options:
      ...
      --agent-browser                Use $HOME/.config/chromium-agent + Default profile
...
SYNC_GUARD_DIR="$BASE/_shared/data/x_tool/guards"
SYNC_GUARD_FILE="$SYNC_GUARD_DIR/sync.guard"
SYNC_AUDIT_DIR="$BASE/_shared/data/x_tool/audit"
SYNC_AUDIT_FILE="$SYNC_AUDIT_DIR/sync_runs.csv"
STALE_GUARD_THRESHOLD_SECONDS=3600

acquire_sync_guard() {
  mkdir -p "$SYNC_GUARD_DIR"
  if [[ -f "$SYNC_GUARD_FILE" ]]; then
    local guard_pid guard_age_seconds
    guard_pid="$(jq -r '.pid // 0' "$SYNC_GUARD_FILE" 2>/dev/null || echo 0)"
    if [[ "$guard_pid" -gt 0 ]] && kill -0 "$guard_pid" 2>/dev/null; then
      echo "SKIP | sync | reason=already-running | held_by_pid=$guard_pid"
      return 1
    fi
    local guard_acquired_at
    guard_acquired_at="$(jq -r '.acquired_at // ""' "$SYNC_GUARD_FILE" 2>/dev/null || echo "")"
    if [[ -n "$guard_acquired_at" ]]; then
      local guard_epoch current_epoch
      guard_epoch="$(date -d "$guard_acquired_at" +%s 2>/dev/null || echo 0)"
      current_epoch="$(date +%s)"
      guard_age_seconds=$((current_epoch - guard_epoch))
      if [[ "$guard_age_seconds" -lt "$STALE_GUARD_THRESHOLD_SECONDS" ]]; then
        echo "SKIP | sync | reason=already-running | held_by_pid=$guard_pid | age=${guard_age_seconds}s"
        return 1
      fi
    fi
    xlog "sync guard | removing stale guard (pid=$guard_pid)"
    rm -f "$SYNC_GUARD_FILE"
  fi
  local guard_data
  guard_data=$(printf '{"pid":%d,"host":"%s","acquired_at":"%s"}' "$$" "$HOSTNAME_SHORT" "$(date -u +%Y-%m-%dT%H:%M:%SZ)")
  if (set -o noclobber; echo "$guard_data" > "$SYNC_GUARD_FILE") 2>/dev/null; then
    return 0
  else
    echo "SKIP | sync | reason=already-running | race-lost"
    return 1
  fi
}
...
append_sync_audit_row() {
  local started_at="$1" finished_at="$2" duration_ms="$3" browser="$4"
  local before_count="$5" after_count="$6" added_count="$7" error_message="$8"
  mkdir -p "$SYNC_AUDIT_DIR"
  if [[ ! -f "$SYNC_AUDIT_FILE" ]] || [[ ! -s "$SYNC_AUDIT_FILE" ]]; then
    echo "started_at,finished_at,duration_ms,host,pid,browser,before_count,after_count,added_count,error_message" > "$SYNC_AUDIT_FILE"
  fi
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,"%s"\n' \
    "$started_at" "$finished_at" "$duration_ms" "$HOSTNAME_SHORT" "$$" \
    "$browser" "$before_count" "$after_count" "$added_count" \
    "${error_message//\"/\"\"}" >> "$SYNC_AUDIT_FILE"
}

parse_sync_counts() {
  local sync_output="$1"
  local added_count after_count
  added_count="$(echo "$sync_output" | grep -oP '\K[0-9]+(?= new bookmarks synced)' || echo 0)"
  after_count="$(echo "$sync_output" | grep -oP '\([0-9]+ total\)' | grep -oP '[0-9]+' || echo 0)"
  local before_count=$((after_count - added_count))
  echo "$before_count $after_count $added_count"
}
...
run_sync() {
  ...
  if ! acquire_sync_guard; then
    return 2
  fi
  trap 'release_sync_guard' EXIT

  if [[ "$agent_browser" -eq 1 ]]; then
    chrome_user_data_dir="$HOME/.config/chromium-agent"
    chrome_profile_directory="Default"
  fi

  local started_at sync_output sync_exit_code error_message=""
  started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

  if [[ -n "$chrome_user_data_dir" || -n "$chrome_profile_directory" ]]; then
    sync_output="$(FT_CHROME_USER_DATA_DIR="$chrome_user_data_dir" \
      FT_CHROME_PROFILE_DIRECTORY="$chrome_profile_directory" \
      ft sync --browser "$browser" --yes 2>&1)" && sync_exit_code=0 || sync_exit_code=$?
  else
    sync_output="$(ft sync --browser "$browser" --yes 2>&1)" && sync_exit_code=0 || sync_exit_code=$?
  fi

  local finished_at duration_ms
  finished_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  local started_epoch finished_epoch
  started_epoch="$(date -d "$started_at" +%s%3N 2>/dev/null || date -d "$started_at" +%s)000"
  finished_epoch="$(date -d "$finished_at" +%s%3N 2>/dev/null || date -d "$finished_at" +%s)000"
  # Trim to milliseconds (13 digits)
  started_epoch="${started_epoch:0:13}"
  finished_epoch="${finished_epoch:0:13}"
  duration_ms=$((finished_epoch - started_epoch))

  local before_count=0 after_count=0 added_count=0
  if [[ "$sync_exit_code" -eq 0 ]]; then
    read -r before_count after_count added_count <<< "$(parse_sync_counts "$sync_output")"
  else
    error_message="$(echo "$sync_output" | tail -1 | head -c 500)"
  fi

  append_sync_audit_row "$started_at" "$finished_at" "$duration_ms" "$browser" \
    "$before_count" "$after_count" "$added_count" "$error_message"

  xlog "sync | exit=$sync_exit_code | before=$before_count | after=$after_count | added=$added_count"
