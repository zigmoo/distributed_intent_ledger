...
#   --osc8     OSC 8 terminal hyperlink (label only, URL hidden)
#   --plain    Bare URL only
...
#   3. /org/platform/data/url_tool/domain_registry.json (scripts library — remote servers)
_resolve_registry() {
...
Options:
  ...
  --osc8      OSC 8 terminal hyperlink output
...
format_output() {
    ...
    case "$OUTPUT_MODE" in
        md)    echo "[$display]($url)" ;;
        osc8)  printf '\033]8;;%s\033\\%s\033]8;;\033\\\n' "$url" "$display" ;;
        plain) echo "$url" ;;
