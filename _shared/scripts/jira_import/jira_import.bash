...
#   jira_import.sh --jql "project=WORK AND status=Backlog" # Import by JQL
#   jira_import.sh --ticket WORK-10927                     # Import single ticket
#   jira_import.sh --ticket WORK-10927,WORK-10926          # Import multiple tickets
...
ensure_token() {
  ...
  if command -v getSecret &>/dev/null; then
    getSecret z_org_jira_personal_access_token 2>&1 | grep -v "^Retrieving" | grep -v "^Secret:" > "$TOKEN_CACHE"
    cat "$TOKEN_CACHE"
  else
...
derive_project() {
  ...
  case "$prefix" in
    "AUTOMATION"|"AUTOMATION SUPPORT") echo "exampleorg-automation" ;;
    "AUTOMATION-POD")                  echo "exampleorg-automation-pod" ;;
    "AUTOMATION/ARCH")                 echo "exampleorg-architecture" ;;
    "ARCH"|"ARCH SUPPORT")             echo "exampleorg-architecture" ;;
    "RESEARCH")                        echo "exampleorg-research" ;;
    "DECOMM")                          echo "exampleorg-decommission" ;;
    "SERVER ADMIN")                    echo "exampleorg-server-admin" ;;
    "GPG Update")                      echo "exampleorg-gpg" ;;
    *)                                 echo "exampleorg-general" ;;
  esac
}
