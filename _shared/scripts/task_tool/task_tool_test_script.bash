...
BASE="$(resolve_dil_base_or_die "$SCRIPTS_DIR")"
...
append_qc_test_run_registry() {
  ...
  local runner="${AGENT_NAME:-${AGENT_ID:-${ASSISTANT_ID:-unknown}}}"
  local machine
  machine="$(hostname -s | tr '[:upper:]' '[:lower:]')"
  local git_commit
  git_commit="$(git -C "$BASE" rev-parse --short HEAD 2>/dev/null || echo unknown)"

  mkdir -p "$QC_DATA_DIR"
  if [[ ! -f "$QC_TEST_RUNS_CSV" ]]; then
    printf '%s\n' 'run_ts_utc,suite,tool,runner,machine,git_commit,mode,single_test,tests_total,passed,failed,skipped,rebuilt,status,log_path,duration_sec' > "$QC_TEST_RUNS_CSV"
  fi
  ...
  QC_TEST_RUNS_CSV="$QC_TEST_RUNS_CSV" \
    python3 -c 'import csv, os
row = [
    os.environ["RUN_TS_UTC"],
    os.environ["SUITE"],
    os.environ["TOOL"],
    os.environ["RUNNER"],
    os.environ["MACHINE"],
    ...
    os.environ["MODE"],
    os.environ["SINGLE_TEST"],
    os.environ["TESTS_TOTAL"],
    os.environ["PASSED"],
    os.environ["FAILED"],
    os.environ["SKIPPED"],
    os.environ["REBUILT"],
    os.environ["STATUS"],
    os.environ["LOG_PATH"],
    os.environ["DURATION_SEC"],
]
with open(os.environ["QC_TEST_RUNS_CSV"], "a", newline="", encoding="utf-8") as handle:
...
test_06_create_work_task() {
  ...
  task_tool --base "$TEST_BASE" create --domain work \
    --task-id "WORK-50001" --title "Test work creation" --project test-suite \
    --priority high --status todo --owner moo \
    --actor test-harness --model test-model 2>&1 || true
  echo "FILE_EXISTS: $(test -f "$TEST_BASE/_shared/domains/work/tasks/active/WORK-50001.md" && echo yes || echo no)"
}

test_07_create_duplicate_fails() {
  make_test_vault
  write_task "WORK-50001" work todo high chore kanban medium "" moo "Already exists"
  write_index_header
  write_index_row WORK-50001 work todo high moo
  write_counter 9001
  write_changelog_header
  task_tool --base "$TEST_BASE" create --domain work \
    --task-id "WORK-50001" --title "Duplicate" --project test-suite 2>&1 || true
  echo "EXIT: $?"
}
...
if (( REBUILD == 1 )); then
  append_qc_test_run_registry "REBUILD"
