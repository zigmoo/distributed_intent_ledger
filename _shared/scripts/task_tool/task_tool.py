...
Subcommands:
  ...
  convert  Re-home a task across domains and ID schemes
...
INDEX_CSV_COLUMNS = ["task_id", "domain", "status", "priority", "owner", "due", "project", "path", "updated", "title", "effort_type"]


def csv_index_path(base: Path) -> Path:
    return base / "_shared" / "_meta" / "task_index.csv"


def read_csv_index(base: Path) -> list[dict[str, str]]:
    csv_file = csv_index_path(base)
...
def write_csv_index(base: Path, rows: list[dict[str, str]]) -> None:
    csv_file = csv_index_path(base)
    csv_file.parent.mkdir(parents=True, exist_ok=True)
    with open(csv_file, "w", newline="", encoding="utf-8") as file_handle:
        writer = csv.DictWriter(file_handle, fieldnames=INDEX_CSV_COLUMNS, extrasaction="ignore")
        writer.writeheader()
        for row in rows:
            writer.writerow(row)


def update_csv_index_row(base: Path, task_id: str, updated_row: dict[str, str]) -> None:
    rows = read_csv_index(base)
    found = False
    ...
    write_csv_index(base, rows)


def append_csv_index_row(base: Path, new_row: dict[str, str]) -> None:
    csv_file = csv_index_path(base)
    csv_file.parent.mkdir(parents=True, exist_ok=True)
...
def render_index_markdown(base: Path) -> None:
    """Render task_index.md from task_index.csv via the J2 template."""
    template_path = base / "_shared" / "_meta" / "task_index.md.j2"
    output_path = base / "_shared" / "_meta" / "task_index.md"
    csv_file = csv_index_path(base)
    if not template_path.exists() or not csv_file.exists():
        return
    lib_dir = base / "_shared" / "scripts" / "lib"
    sys.path.insert(0, str(lib_dir))
    try:
        from script_forge_renderer import render_csv_to_markdown
        render_csv_to_markdown(csv_file, template_path, output_path)
    except ImportError:
        pass
    finally:
...
def url_tool_path(base: Path) -> Path | None:
    script_dir = Path(__file__).resolve().parent
    candidates = [
        script_dir / "bin" / "url_tool",
        script_dir / "url_tool" / "url_tool.bash",
        script_dir / "url_tool.sh",
    ]
...
def markdown_task_link(base: Path, task_id: str) -> str:
    url_tool = url_tool_path(base)
    if url_tool is None:
        return task_id
    env = os.environ.copy()
    env["URL_TOOL_REGISTRY"] = str(base / "_shared" / "_meta" / "domain_registry.json")
    proc = subprocess.run(
        [str(url_tool), "ticket", task_id],
        capture_output=True,
        text=True,
        env=env,
    )
    return proc.stdout.strip() if proc.returncode == 0 else task_id
...
def render_task_search_rows(base: Path, rows: list[dict[str, str]]) -> str:
    nozzle = Path(__file__).resolve().parent / "lib" / "nozzles" / "ticket_report.py"
    template = Path(__file__).resolve().parent / "j2_templates" / "ticket_report_console.txt.j2"
    report_rows = [
        {
            "ticket": row["task_id"],
            "status": row["status"],
    ...
    if nozzle.exists():
        proc = subprocess.run(
            [sys.executable, str(nozzle), "--base", str(base), "--template", str(template)],
            input=json.dumps(report_rows),
            capture_output=True,
            text=True,
        )
    ...
    lines = [
        f"{row['ticket']:<14}  {row['status']:<13}  {row['priority']:<8}  {row['owner']:<20}  {row['updated']:<10}  {row['summary']}"
...
def update_index_row(index_file: Path, task_id: str, new_row: str) -> None:
    base = index_file.parent.parent.parent
    columns = [column.strip() for column in new_row.strip().strip("|").split("|")]
    core_column_names = ["task_id", "domain", "status", "priority", "owner", "due", "project", "path", "updated"]
    if len(columns) >= 9:
        row_dict = dict(zip(core_column_names, columns[:9]))
        existing_rows = read_csv_index(base)
        for existing_row in existing_rows:
            if existing_row["task_id"] == task_id:
                row_dict["title"] = existing_row.get("title", "")
                row_dict["effort_type"] = existing_row.get("effort_type", "medium")
                break
        else:
            row_dict.setdefault("title", "")
            row_dict.setdefault("effort_type", "medium")
        update_csv_index_row(base, task_id, row_dict)
        render_index_markdown(base)
...
"""
        ...
        # Update index (CSV-primary, then render markdown)
        append_csv_index_row(args.base, {
            "task_id": task_id, "domain": domain_name, "status": status,
            "priority": priority, "owner": owner, "due": due or "",
            "project": project, "path": task_rel, "updated": date_utc,
            "title": title, "effort_type": effort_type,
        })
        render_index_markdown(args.base)
...
def verify_external_ticket(ticket_id: str) -> tuple[bool, str]:
    proc = subprocess.run(
        ["jira_tool", "status", ticket_id],
...
def cmd_convert(args: argparse.Namespace) -> int:
    source_id = args.source_task_id.strip()
    to_domain = args.to_domain.strip()
    to_id = (args.to_id or "").strip()
    owner_override = (args.owner or "").strip()

    registry = load_registry(args.base)
    domains = registry.get("domains", {})
    if to_domain not in domains:
        fail(2, f"unknown domain: {to_domain}")
    target_conf = domains[to_domain]
    target_id_mode = target_conf.get("id_mode", "auto")

    if target_id_mode == "external" and not to_id:
        fail(2, f"target domain {to_domain} has id_mode=external, --to-id required")

    source_file = task_file_for_id(args.base, source_id)
    source_text = source_file.read_text(encoding="utf-8")
    source_fm, _ = parse_frontmatter(source_text)
    source_domain = source_fm.get("domain", "")

    if source_domain == to_domain and not to_id:
        fail(2, f"source task {source_id} is already in domain {to_domain}")

    # Verify external ticket exists before committing
    if target_id_mode == "external" and to_id:
        found, ticket_info = verify_external_ticket(to_id)
        if not found:
            fail(2, f"external ticket {to_id} not found in Jira: {ticket_info}")
        eprint(f"VERIFIED | {ticket_info}")

    script_dir = Path(__file__).resolve().parent
    actor = detect_assistant(args.actor, script_dir)
    model = detect_model(args.model)

    target_task_dir = resolve_domain_task_dir(args.base, to_domain)
    target_active = target_task_dir / "active"
    if not target_active.is_dir():
        fail(4, f"Missing active task directory: {target_active}")
    ...
    if not args.dry_run:
        lock = DirLock(lock_path, max_retries=30)
        if not lock.acquire():
            fail(4, f"Could not acquire lock after {lock.max_retries} attempts")

    try:
        new_id = to_id
        new_next_id = None
        id_prefix = target_conf.get("id_prefix", "")

        if target_id_mode == "auto" and not to_id:
            new_next_id = read_counter(counter_file, id_prefix)
            if new_next_id is None:
                fail(4, f"Invalid next_id for prefix {id_prefix} in {counter_file}")
            for bump in range(201):
                new_id = f"{id_prefix}-{new_next_id}"
                candidate = target_active / f"{new_id}.md"
        ...
        target_file = target_active / f"{new_id}.md"
        stub_merge = target_file.exists()

        new_owner = owner_override or target_conf.get("default_owner", "moo")
        now_utc = dt.datetime.now(dt.timezone.utc)
        date_utc = now_utc.strftime("%Y-%m-%d")
        ts_utc = now_utc.strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        fm_updates = {
            "task_id": new_id,
            "domain": to_domain,
            "owner": yaml_quote(new_owner),
            "updated": date_utc,
        }

        merge_label = " (merged into existing stub)" if stub_merge else ""
        convert_note = f"\n- {ts_utc}: Converted from {source_id} ({source_domain}) via task_tool convert{merge_label}. Original domain: {source_domain}.\n"
        retire_note = f"\n- {ts_utc}: Retired — converted to {new_id} ({to_domain}) via task_tool convert.\n"

        if args.dry_run:
            print("DRY RUN")
            if stub_merge:
                print(f"Would merge into existing stub: {target_file}")
            else:
                print(f"Would copy: {source_file} -> {target_file}")
            print(f"Would retire source: {source_file} (status -> retired)")
            print(f"Frontmatter updates on target: {fm_updates}")
            print(f"New owner: {new_owner}")
            if target_id_mode == "auto" and new_next_id is not None:
                print(f"Would update counter {id_prefix} next_id: {new_next_id} -> {new_next_id + 1}")
            return 0

        # Copy source content to target (overwrites stub if present)
        import shutil
        shutil.copy2(str(source_file), str(target_file))
        update_task_frontmatter(target_file, fm_updates)

        with target_file.open("a", encoding="utf-8") as f:
            f.write(convert_note)

        # Retire source with forward pointer
        update_task_frontmatter(source_file, {"status": "retired", "updated": date_utc})
        with source_file.open("a", encoding="utf-8") as f:
            f.write(retire_note)

        # Update children that reference the old task_id as parent
        children_updated = 0
        for dname, dconf in domains.items():
            td = dconf.get("task_dir", "")
            td_path = resolve_registered_path(args.base, td)
            child_active = td_path / "active"
            if not child_active.is_dir():
                continue
            for child_file in child_active.glob("*.md"):
                if child_file == source_file or child_file == target_file:
                    continue
                child_text = child_file.read_text(encoding="utf-8")
                child_fm, _ = parse_frontmatter(child_text)
                if child_fm.get("parent_task_id") == source_id:
                    update_task_frontmatter(child_file, {"parent_task_id": force_quote(new_id)})
                    children_updated += 1

        if target_id_mode == "auto" and new_next_id is not None:
            update_counter(counter_file, id_prefix, new_next_id + 1, actor, model)
        ...
        # Changelog
        cl_rows = [
            f"| {ts_utc} | {actor} | {model} | {new_id} | convert | converted from {source_id} ({source_domain}) to {to_domain}{merge_label} | task_tool convert |",
            f"| {ts_utc} | {actor} | {model} | {source_id} | update | status: {source_fm.get('status', 'todo')}->retired (converted to {new_id}) | task_tool convert |",
        ]
        if children_updated:
            cl_rows.append(
                f"| {ts_utc} | {actor} | {model} | {new_id} | update | updated {children_updated} child parent_task_id refs {source_id}->{new_id} | task_tool convert |"
            )
    ...
    stub_msg = " (stub merged)" if stub_merge else ""
    print(f"OK | {new_id} | {to_domain} | converted from {source_id} | source {source_id} retired{stub_msg}")
    return 0
...
def cmd_rebuild_index_impl(base: Path) -> None:
    ...
    for directory in task_dirs:
        for task_file in sorted(directory.glob("*.md")):
            if task_file.name == "index.md":
                continue
            frontmatter, _ = parse_frontmatter(task_file.read_text(encoding="utf-8"))
            relative_path = str(task_file.relative_to(base))
            csv_rows.append({
                "task_id": frontmatter.get("task_id", task_file.stem),
                "domain": frontmatter.get("domain", ""),
                "status": frontmatter.get("status", ""),
                ...
                "updated": frontmatter.get("updated", ""),
                "title": frontmatter.get("title", task_file.stem),
                "effort_type": frontmatter.get("effort_type", "medium"),
            })
            match = re.match(r"^DIL-(\d+)$", frontmatter.get("task_id", ""))
            if match:
                number = int(match.group(1))
                if number > max_dil:
                    max_dil = number

    write_csv_index(base, csv_rows)
    render_index_markdown(base)
    ...
    print(f"Rebuilt index: {csv_index_path(base)}")
    print(f"Rendered markdown: {base / '_shared' / '_meta' / 'task_index.md'}")
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    id_rules.setdefault("work", ("WORK", "external"))
...
def cmd_search(args: argparse.Namespace) -> int:
    ...
    print(render_task_search_rows(args.base, rows), end="")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="task_tool",
        description="Fast task discovery, review, and handoff tooling. Human-facing task IDs are formatted with url_tool links.",
    )
    parser.add_argument("--base", required=True, help="Resolved DIL base path from task_tool.bash")
    ...
    convert = subparsers.add_parser("convert", help="Re-home a task across domains and ID schemes")
    convert.add_argument("source_task_id", help="Existing task ID to convert (e.g., DIL-1517)")
    convert.add_argument("--to-domain", required=True, dest="to_domain", help="Target domain key from domain_registry.json")
    convert.add_argument("--to-id", dest="to_id", help="New task ID (required when target domain id_mode is external)")
    convert.add_argument("--owner", help="Override owner (defaults to target domain default_owner)")
    convert.add_argument("--actor")
