#!/usr/bin/env bash
# memory_tool_test_script.bash — golden file diff test suite for memory_tool
...
TEST_SCRIPT_NAME="memory_tool_test_script"
...
normalize() {
  sed \
    ...
    -e 's/created_at: .*/created_at: TIMESTAMP/g' \
    -e 's/-[0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\}\.md/-DATE.md/g'
}
...
setup_test_dil() {
  ...
  mkdir -p "$test_base/_shared/logs/memory_tool"
  mkdir -p "$test_base/_shared/domains/personal/memories"
  mkdir -p "$test_base/_shared/domains/work/memories"
...
DREOF
  ...
  # Vault index stub
  touch "$test_base/_shared/_meta/vault_index.md"
}

# --- Tests ---

test_01_help() {
  python3 "$SCRIPT_DIR/memory_tool.py" --help
}

test_02_create_help() {
  python3 "$SCRIPT_DIR/memory_tool.py" create --help
}

test_03_create_dry_run() {
  local test_base="$TEST_WORKSPACE/test_03"
  setup_test_dil "$test_base"
  python3 "$SCRIPT_DIR/memory_tool.py" create \
    --type observation \
...
test_04_create_actual() {
  ...
  echo "--- files created ---"
  find "$test_base/_shared" -name "*.md" -not -name "vault_index.md" | sort | sed "s|$test_base/||"
}

test_05_relocate_help() {
  python3 "$SCRIPT_DIR/memory_tool.py" relocate --help
}

test_06_remove_help() {
  python3 "$SCRIPT_DIR/memory_tool.py" remove --help
}

# --- Run tests ---

run_test 1 "help output"                test_01_help
run_test 2 "create subcommand help"     test_02_create_help
run_test 3 "create dry run"             test_03_create_dry_run
run_test 4 "create actual note"         test_04_create_actual
run_test 5 "relocate subcommand help"   test_05_relocate_help
run_test 6 "remove subcommand help"     test_06_remove_help
