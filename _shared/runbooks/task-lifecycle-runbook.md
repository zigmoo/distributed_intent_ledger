...
### 0. ID-Only Dispatch (when provided by operator)

If the operator provides an ordered list of task IDs as the dispatch mechanism:

- Execute those IDs in that exact order.
- Do not ask for additional scoping/context unless blocked by a hard execution error.
- If details are sparse, use the parent task execution notes as working context.
- After completing one task, proceed directly to the next ID.
...
```bash
# 1. Create in Jira
jira_tool create --summary "Fix broken cron job on dv-work-svr-01"
# OK | WORK-11900 | created

# 2. Create DIL task file
_shared/scripts/create_task.sh \
  --domain work --task-id WORK-11900 \
  --title "Fix broken cron job on dv-work-svr-01" \
  --project exampleorg-server-admin \
  --priority high --work-type bug --task-type kanban --effort-type low

# 3. Transition to In Progress
jira_tool transition WORK-11900 "In Progress"

# 4. Do the work...

# 5. Add evidence
jira_tool comment WORK-11900 '{noformat}$ crontab -l | grep backup
0 2 * * * /home/platform/scripts/backup.sh{noformat}'

# 6. Transition to Done
jira_tool transition WORK-11900 "Done"
