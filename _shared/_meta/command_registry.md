...
| test all, tool forge test all, run all tests, full test run | `tool_forge_test_all` | Run ALL Tool Forge test suites in a single pass. Discovers *_test_script.bash across all drawers, handles infra-dependent tools, reports aggregate pass/fail. Use `--json` for machine output, `--skip <tool>` to exclude, `--only <tool>` to focus. |
...
## Work Domain Tools (Exampleorg)

Located in `/org/platform/scripts/bin/`. Only available in work context.

| trigger | command | description |
|---|---|---|
| jira comment, update jira, jira | `jira_tool comment WORK-XXXXX "text"` | Post comment to Jira ticket |
| jira status | `jira_tool status WORK-XXXXX` | One-line Jira ticket status |
| jira transition, move ticket | `jira_tool transition WORK-XXXXX "In Progress"` | Transition Jira ticket (handles multi-step chaining) |
| jira create, create ticket | `jira_tool create --summary "Title"` | Create a Jira ticket |
| jira subtask | `jira_tool create-subtask WORK-XXXXX --summary "Title"` | Create Jira subtask with watcher inheritance |
| jira json, jira manifest | `jira_tool json <manifest.json>` | Agent-first JSON mode for Jira |
| jira edit | `jira_tool edit WORK-XXXXX --field value` | Edit Jira ticket fields |
...
| deploy app | `deployApp` | Deploy Platform application |
| deploy config | `deployConfig` | Deploy Platform configuration |
| run platform job | `runPlatformJob` | Execute a Platform job |
| platform status | `platformStatus` | Check Platform server/job status |
...
| start deploytool, deployinator start | `deploytool_ctl start` | Build and start DeployTool in production mode (tmux session) |
| stop deploytool, deployinator stop | `deploytool_ctl stop` | Stop DeployTool and free port |
| restart deploytool | `deploytool_ctl restart` | Stop then start DeployTool |
| deploytool status | `deploytool_ctl status` | Check if DeployTool is running (PID, URL, log) |
| deploytool dev | `deploytool_ctl dev` | Start DeployTool in dev mode (turbopack) |
...
| jira link | `jira_tool link WORK-XXXXX <URL\|ISSUE_KEY> [TITLE\|TYPE]` | Add web or issue link to Jira ticket |
...
- **Never hardcode user-specific paths** (for example `$HOME/...`) as default DIL base resolution.
...
- **SMAX create-request vs create-change**: use `create-request` for consultation/service requests (e.g. ask TUG to decomm servers), use `create-change` for change requests (e.g. deploying data-processing jobs, Control-M changes)
- **SMAX ChangeReason enum**: use `BusinessRequirement` (not free text)
- Logs for smax_tool: `/org/platform/logs/smax_tool/`
- **DeployTool paths**: App `/org/platform/scripts/node/apps/DeployTool/`, Data `/org/platform/data/DeployTool/`, Logs `/org/platform/logs/DeployTool/`
- **DeployTool port**: 3001 (env: `WORK_PORT`), tmux session: `DeployTool`
